...
### 1.3.1
- **Fix:** Location of `llms.txt` and `ai.txt` now correctly placed at the website root.  
- **Fix:** `ai.txt` now contains the correct permissions type information.  
- **Added:** Feature to immediately re-generate `llms.txt` and `ai.txt` files.  
- **Added:** Allow AI training toggle.  
- **Added:** Content licence defaults to **CC BY-SA 4.0**, unless otherwise changed.  
- **Added:** Ability to disable "File Update Frequency".  
- **Updated:** Settings UI to present a more intuitive interface and options.
